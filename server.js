...
app.post("/api/create-video", upload.fields([{ name: "images", maxCount: 20 }]), async (req, res) => {
  try {
    const images = req.files?.images || [];
    const imageUrls = images.map(publicUrl);
    if (!imageUrls.length) return res.status(400).json({ error: "En az 1 görsel yükle." });
    ...
    res.status(n8nRes.ok ? 200 : 502).json({
      ok: n8nRes.ok,
      message: "Panel isteği n8n'e gönderdi.",
      uploaded: { imageUrls },
      request: {
        clipCount: req.body.clipCount,
        voiceMode: payload.voiceMode,
        musicMode: payload.musicMode,
        hasVoiceText: Boolean(payload.voiceText),
        hasMusicPrompt: Boolean(payload.musicPrompt)
      },
      n8n: data
    });
  } catch (err) {
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(`AI Video Panel clean çalışıyor: http://0.0.0.0:${PORT}`);
